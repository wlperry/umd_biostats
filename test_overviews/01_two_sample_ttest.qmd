...
(`r round(mean_se_by_lake$mean[mean_se_by_lake$lake == "S 07"], 2)` ± `r round(mean_se_by_lake$se[mean_se_by_lake$lake == "S 07"], 2)` mm vs. `r round(mean_se_by_lake$mean[mean_se_by_lake$lake == "NE 14"], 2)` ± `r round(mean_se_by_lake$se[mean_se_by_lake$lake == "NE 14"], 2)` mm, respectively; two-sample t-test: t(`r round(df)`) = `r t_statistic`, p \< 0.001). This represents an approximately `r round(percent_diff, 1)`% difference in total length between the two populations."
